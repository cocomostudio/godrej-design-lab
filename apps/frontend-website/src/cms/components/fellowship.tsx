...
export class Fellowship {
	static id = "fellowships.fellowship-v1"

	static process_node ( props ) {
		return shallow_clone_props( props )
	}

	static Renderer ({ name, description, projects }) {
		const [ isOpen, setIsOpen ] = useState( false )

		return <div className="fellowship-programme mt-4 _mt-10 [.fellowship-programme:first-child]:mt-0 pt-4 border-t border-black">
			<div className="flex justify-between items-center cursor-pointer" onClick={ () => setIsOpen( v => !v ) }>
				<h2 className="text-h4 md:text-h1 font-bold text-secondary">{ name }</h2>
				<span className="text-h4 md:text-h1 font-thin">{ isOpen ? "—" : "+" }</span>
			</div>

			<div className={ `not-interpolate:grid ${ isOpen ? "not-interpolate:grid-rows-[1fr] interpolate:h-auto" : "not-interpolate:grid-rows-[0fr] interpolate:h-0 delay-250" } not-interpolate:transition-all interpolate:transition-[height] !duration-450 !ease-vaul` }>
				<div className={ `overflow-hidden ${ isOpen ? "delay-300" : "opacity-0 pointer-events-none" } transition-opacity duration-250` }>
					...
					{ projects.map( ( { description, cover, fellows, link }, i ) => <div key={ i } className="fellowship-project mt-3.5 md:mt-4 flex flex-wrap md:border-b border-black md:last:border-none md:pb-4">
						<figure className="w-2c-1g md:w-1c lg:w-1c-2g rounded-md overflow-hidden">
							<img src={ CMS_PUBLIC_DIR_URL + cover.file.url } className="aspect-square object-cover" />
						</figure>
						<div className="md:order-1 ml-1g w-3c-2g md:w-2c-1g lg:ml-2g lg:w-3c-4g">
