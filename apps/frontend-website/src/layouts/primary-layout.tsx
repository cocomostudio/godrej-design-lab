...
function HeaderNavigation ( { isVisible = false, className = "" }: HeaderNavigationProps ) {
	return <header className={ `bg-primary relative after:absolute after:top-0 after:right-0 after:w-1/3 after:h-full md:after:bg-secondary ${ className }` }>
		<nav className="md:container md:grid-layout">
			...
			<div className={ `start-col-2 end-gutter-5 lg:end-col-7 text-secondary transition-opacity duration-250 delay-200 ease-out ${ isVisible ? "opacity-100" : "opacity-0" }` }>
			...
			<div className="start-col-6 end-col-last md:ml-1g md:pl-6 lg:start-col-8 lg:ml-[calc(var(--column-width)/2)] bg-secondary text-primary z-10">
				<div className="container py-8 bg-secondary">
					<h2 className={ `border-t border-primary border-solid md:border-none pt-4 md:pt-0 text-p font-bold uppercase transition-opacity duration-250 delay-200 ease-out ${ isVisible ? "opacity-100" : "opacity-0" }` }>Latest</h2>
					<ul className={ `mt-6 space-y-3 transition-opacity duration-250 delay-200 ease-out ${ isVisible ? "opacity-100" : "opacity-0" }` }>
		...
		<nav className="bg-primary relative after:absolute after:top-0 after:right-0 after:w-1/3 after:h-full md:after:bg-secondary">
			<div className="md:container md:grid-layout">
				<div className="py-6 lg:py-9 start-col-1 end-col-5 lg:end-col-7">
					...
					<div className="mt-10 md:mt-15.5 lg:mt-28 text-secondary">
				...
				<div className="start-gutter-5 lg:start-col-8 end-col-last md:-ml-1g lg:ml-0 py-4.5 md:pl-4 md:py-6 lg:pl-12 lg:py-10 bg-secondary text-primary z-10">
					<div className="container md:flex flex-col justify-between h-full bg-secondary">
						<Link to={ "/" }>
							<GodrejLogo className="md:w-1c h-auto fill-primary" />
						</Link>
						<div className="mt-10">
