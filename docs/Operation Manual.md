...
Kalpesh Patel <kalpeshp@godrej.com>
...
This introduction provides a high level technical overview of the project. The scope of this document is technical and focuses on the operational aspects of the GDL project. If the reader is curious for more context on the project itself, or its history, a good point of contact for enquiry is Henry Skupniewicz (Head of Godrej Design Lab) at Godrej & Boyce Mfg. Co. Ltd., the owner and the sponsor of this project.

This project was developed and built by Cocomo Studio, a design agency that builds and develops a wide variety of design and technology projects.
...
Before diving in further, let’s look at and internalize the bird's eye view of the project and its architecture.

![Architecture diagram](Architecture.png)

At a glance the stack is a classic three-tier Frontend + API+ DB arrangement, fully inside a single AWS account within the same VPC. Please note that this architecture's bird's eye view does not include Godrej-specific security, firewall and other implementations.

Let’s look at the application architecture bill of materials below.
...
The code repository is available in the cocomo user on the home folder of each of the CMS and FRONTEND instances under the folder named **repo**.

For the audit trail, and in academic interest, we simply used SCP to transfer all the code from the developer's computers into the EC2 instances after logging in via the Godrej VPN[^4].

In the case that the Godrej team has to spin up a new instance and transfer the repository there, that is outside the scope of this document and we assume that the admin has necessary means to move these files in a manual and Godrej-compliant way.
...
**Step 3.3 -- configuring environment variables**
...
2. Birds eye view of ALB, Frontend EC2, Strapi EC2, RDS, S3, and CloudFront with the concrete hostnames, IP addresses and systems involved
